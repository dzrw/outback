describe('the value binding', function() {
	...
	describe('should work with select controls with multiple="multiple"', function() {
		...
		describe('when the control has pre-selected values', function() {
			beforeEach(function() {
				this.model = new AModel();
				this.view = new FixtureView({
					model: this.model
				...
				_.extend(this.view, {
					innerHtml: "<select type='text' multiple='multiple' data-bind='value: @car'> \
			...
			it('should initialize the model with correct pre-selected elements', function() {
				this.el.trigger('change');
				
				expect(this.model.get('car').length).toEqual(2);
				expect(this.model.get('car')).toContain('volvo');
				expect(this.model.get('car')).toContain('mercedes');
			})

		})

		describe('when the control does not have pre-selected values', function() {
	...
	describe('helps prevent XSS attacks', function() {
		...
		it('should allow you to shoot yourself in the foot', function() {
			this.view.modelBindings['#anchor input'].valueOptions = {
				escape: false
			};
